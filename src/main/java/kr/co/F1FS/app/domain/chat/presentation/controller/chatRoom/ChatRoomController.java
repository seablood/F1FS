package kr.co.F1FS.app.domain.chat.presentation.controller.chatRoom;
...
public class ChatRoomController {
    ...
    public ResponseEntity<List<ResponseChatRoomDTO>> getChatRoomAll(@RequestParam(value = "page", defaultValue = "0") int page,
                                                             @RequestParam(value = "size", defaultValue = "10") int size,
                                                             @RequestParam(value = "condition", defaultValue = "new") String condition){
        Page<ResponseChatRoomDTO> newPage = chatRoomUseCase.getChatRoomAll(page, size, condition);
        return ResponseEntity.status(HttpStatus.OK).body(newPage.getContent());
    }

    @GetMapping("/find-subscribe")
    @Operation(summary = "채팅방 검색(참여 중인 채팅방)", description = "참여 중인 모든 채팅방을 정렬 방식에 따라 반환")
    public ResponseEntity<List<ResponseChatRoomDTO>> getChatRoomListByIdIn(@RequestParam(value = "page", defaultValue = "0") int page,
                                                                           @RequestParam(value = "size", defaultValue = "10") int size,
                                                                           @RequestParam(value = "condition", defaultValue = "new") String condition,
                                                                           @AuthenticationPrincipal PrincipalDetails principalDetails){
        Page<ResponseChatRoomDTO> newPage = chatRoomUseCase.getChatRoomListByIdIn(page, size, condition, principalDetails.getUsername());
        return ResponseEntity.status(HttpStatus.OK).body(newPage.getContent());
    }
