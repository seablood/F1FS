...
import kr.co.F1FS.app.domain.note.application.service.NoteService;
...
public class NoteController {
    ...
    public ResponseEntity<ResponseNoteDTO> save(@Valid @RequestBody CreateNoteDTO dto, @RequestParam String nickname,
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(noteService.save(dto, principalDetails.getUser(), nickname));
    }
    ...
    public ResponseEntity<List<ResponseSimpleNoteDTO>> getNoteByToUser(@RequestParam(value = "page", defaultValue = "0") int page,
                                                                       ...
                                                                       @AuthenticationPrincipal PrincipalDetails principalDetails){
        Page<ResponseSimpleNoteDTO> newPage = noteService.getNoteByToUser(principalDetails.getUser(), page, size);
    ...
    public ResponseEntity<List<ResponseSimpleNoteDTO>> getNoteByFromUser(@RequestParam(value = "page", defaultValue = "0") int page,
                                                                       @RequestParam(value = "size", defaultValue = "10") int size,
                                                                       @AuthenticationPrincipal PrincipalDetails principalDetails){
        Page<ResponseSimpleNoteDTO> newPage = noteService.getNoteByFromUser(principalDetails.getUser(), page, size);
    ...
    public ResponseEntity<ResponseNoteDTO> findById(@PathVariable Long id){
        return ResponseEntity.status(HttpStatus.OK).body(noteService.findByIdDTO(id));
    }
    ...
    public ResponseEntity<ResponseNoteDTO> modify(@Valid @RequestBody ModifyNoteDTO dto, @PathVariable Long id){
        return ResponseEntity.status(HttpStatus.OK).body(noteService.modify(id, dto));
    }
